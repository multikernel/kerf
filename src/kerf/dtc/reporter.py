...
from ..models import GlobalDeviceTree, ValidationResult
...
class ValidationReporter:
    ...
    def _generate_text_report(
        self, result: ValidationResult, tree: GlobalDeviceTree, verbose: bool = False  # pylint: disable=unused-argument
    ) -> str:
    ...
    def _format_hardware_inventory(self, tree: GlobalDeviceTree) -> List[str]:
        ...
        lines.append(
            f"    Host reserved: {cpus.host_reserved[0]}-{cpus.host_reserved[-1]} ({host_count} CPUs, {host_percent:.0f}%)"
        )
        lines.append(
            f"    Memory pool: {cpus.available[0]}-{cpus.available[-1]} ({available_count} CPUs, {memory_pool_percent:.0f}%)"
        )
        ...
        network_devices = sum(
            1 for d in tree.hardware.devices.values() if "ethernet" in d.compatible.lower()
        )
        storage_devices = sum(
            1 for d in tree.hardware.devices.values() if "nvme" in d.compatible.lower()
        )
    ...
    def _format_instance_allocations(self, tree: GlobalDeviceTree) -> List[str]:
        ...
        for name, instance in tree.instances.items():
            ...
            cpu_range = (
                f"{instance.resources.cpus[0]}-{instance.resources.cpus[-1]}"
                if instance.resources.cpus
                else "none"
            )
            ...
            memory_percent = (
                instance.resources.memory_bytes / tree.hardware.memory.memory_pool_bytes
            ) * 100
            lines.append(
                f"    Memory: {memory_gb:.0f}GB at {hex(instance.resources.memory_base)} ({memory_percent:.0f}% of pool)"
            )
    ...
    def _format_resource_utilization(self, tree: GlobalDeviceTree) -> List[str]:
        ...
        # Calculate totals
        total_cpus_allocated = sum(
            len(instance.resources.cpus) for instance in tree.instances.values()
        )
        total_memory_allocated = sum(
            instance.resources.memory_bytes for instance in tree.instances.values()
        )
        ...
        lines.append(
            f"  CPUs: {total_cpus_allocated}/{available_cpus} allocated ({cpu_percent:.0f}%), {available_cpus - total_cpus_allocated} free"
        )

        memory_gb_allocated = total_memory_allocated / (1024**3)
        memory_gb_total = memory_pool_bytes / (1024**3)
        lines.append(
            f"  Memory: {memory_gb_allocated:.0f}/{memory_gb_total:.0f} GB allocated ({memory_percent:.0f}%), {memory_gb_total - memory_gb_allocated:.0f} GB free"
        )

        # Device utilization
        total_devices = len(tree.hardware.devices)
        allocated_devices = sum(
            len(instance.resources.devices) for instance in tree.instances.values()
        )

        # Calculate device-specific utilization
        network_devices = sum(
            1 for device in tree.hardware.devices.values() if "ethernet" in device.compatible
        )
        storage_devices = sum(
            1 for device in tree.hardware.devices.values() if "nvme" in device.compatible
        )

        allocated_network = sum(
            1
            for instance in tree.instances.values()
            for device_ref in instance.resources.devices
            if "_vf" in device_ref
        )
        allocated_storage = sum(
            1
            for instance in tree.instances.values()
            for device_ref in instance.resources.devices
    ...
    def generate_json_report(
        self, result: ValidationResult, tree: GlobalDeviceTree
    ) -> Dict[str, Any]:
