...
class DeviceTreeParser:
    ...
    def _extract_brace_section(self, dts_content: str, start_keyword: str) -> Optional[str]:
        """Extract any section content with proper brace matching."""
        import re

        section_start = re.search(rf'{start_keyword}\s*\{{', dts_content)
    ...
    def _extract_resources_section(self, dts_content: str) -> Optional[str]:
        return self._extract_brace_section(dts_content, "resources")
    ...
    def _parse_cpus_from_dts(self, dts_content: str) -> CPUAllocation:
        ...
        cpus_text = self._extract_brace_section(resources_text, "cpus")
        if not cpus_text:
            raise ParseError("Missing 'cpus' property in /resources")

        cpus_available_match = re.search(r'available\s*=\s*<([^>]+)>', cpus_text)
        if not cpus_available_match:
            raise ParseError("Missing 'available' in cpus")

        host_reserved = []
        cpus_host_reserved_match = re.search(r'host-reserved\s*=\s*<([^>]+)>', cpus_text)
        if cpus_host_reserved_match:
            host_reserved = [int(x.strip()) for x in cpus_host_reserved_match.group(1).split()]

        available = [int(x.strip()) for x in cpus_available_match.group(1).split()]
        if available:
            total = max(available) + 1
        else:
            total = 0
    ...
    def _parse_memory_from_dts(self, dts_content: str) -> MemoryAllocation:
        ...
        mem_text = self._extract_brace_section(resources_text, "memory")

        host_reserved_bytes = 0
        memory_host_reserved_match = re.search(r'host-reserved-bytes\s*=\s*<([^>]+)>', mem_text)
        if memory_host_reserved_match:
            host_reserved_bytes = self._parse_hex_value(memory_host_reserved_match.group(1))

        memory_base_match = re.search(r'memory-pool-base\s*=\s*<([^>]+)>', mem_text)
        if not memory_base_match:
            raise ParseError("Missing 'memory-pool-base' property in /resources")

        memory_bytes_match = re.search(r'memory-pool-bytes\s*=\s*<([^>]+)>', mem_text)
        if not memory_bytes_match:
            raise ParseError("Missing 'memory-pool-bytes' property in /resources")
