...
def is_dts_text(path) -> bool:
    """True when the file is a DTS rendering rather than a DTB."""
    with open(path, "rb") as f:
        head = f.read(64)
    return head.lstrip().startswith(b"/dts-v1/")
...
class DeviceTreeParser:
    ...
    def dts_from_dtb(self, dtb_data: bytes) -> str:
        """Render a DTB as DTS text for reading; kerf does not parse it back."""
        try:
            self.fdt = libfdt.Fdt(dtb_data)
        except Exception as e:
            raise ParseError(f"Failed to render DTB as DTS: {e}") from e
        return "/dts-v1/;\n\n" + "\n".join(self._fdt_to_dts_recursive(0, 0)) + "\n"
    ...
    def _property_to_dts(self, name: str, data: bytes, indent: str) -> str:
        ...
        # A NUL-terminated printable run is a string even when it is 4 or 8
        # bytes long, which is how "pci\0" would otherwise print as a cell
