...
def demonstrate_resource_analysis():
    ...
    for name, instance in tree.instances.items():
        ...
        print(
            f"    Memory: {memory_gb:.0f}GB ({memory_gb/(memory_pool_bytes/(1024**3))*100:.1f}% of pool)"
        )
    ...
    print(
        f"  Memory: {total_memory_allocated/(1024**3):.0f}GB/{memory_pool_bytes/(1024**3):.0f}GB ({memory_percent:.1f}%)"
    )
    ...
    if unallocated_cpus > 0:
        print(
            f"  Unallocated: {unallocated_cpus} CPUs, {unallocated_memory/(1024**3):.0f}GB memory"
        )
...
def main():
    ...
    print("✓ Resource analysis: PASSED")
    print("✓ Error reporting: PASSED")

    print("\n🎉 kerf core functionality demonstration complete!")
    print("\nKey Features Demonstrated:")
    print("  • Comprehensive validation of multikernel configurations")
    print("  • CPU, memory, and device allocation conflict detection")
    print("  • Resource utilization analysis and reporting")
    print("  • Detailed error messages with actionable suggestions")
    print("  • Support for various workload types and configurations")

    print("\nNext Steps:")
    print("  1. Implement DTS/DTB parsing for real device tree files")
    print("  2. Add instance extraction and DTB generation")
    print("  3. Integrate with kernel sysfs interface")
    print("  4. Add support for dynamic configuration updates")
